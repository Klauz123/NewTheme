...
$(document).on('click','.close-search-control',function(){
>>>>>>> parent of b97db23... javascripted
...
$(document).ready(function() {
  ...
  if ($('.accordion')[0]) {
    $('.accordion-title').on('click', function() {
      if ($(window).width() < 768) {
        $(this).next().toggle().prev().toggleClass('actives');
        return false;
      };
    });
  }
  ...
  $('#mobilemenubutton').bind('click', function() {
    ...
    if ($('div#' + otherMenu).is(':visible')) {
      $(otherIcon).removeClass(hoverClass);
      $('div#' + otherMenu).stop().slideUp('medium');
    }
    if ($(clickedIcon).hasClass(hoverClass)) {
      $('div#' + openMenu).stop().slideUp('medium');
      $(clickedIcon).removeClass(hoverClass);
    } else {
      $('div#' + openMenu).stop().slideDown('medium');
      $(clickedIcon).addClass(hoverClass);
    }
  });
